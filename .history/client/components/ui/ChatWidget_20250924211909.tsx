...
export default function ChatWidget({ role = "student" }: { role?: "teacher" | "student" }) {
  ...
  useEffect(() => {
    ...
    const onParticipants = (list: Participant[]) => {
      const prevCount = participants.length;
      const newCount = list.length;
      
      // If a new participant joined (count increased), clear chat
                ...
                {messages.map((m, index) => {
                  ...
                  return (
                    <div key={m.id} className="space-y-1">
                      ...
                      <div className={`inline-block max-w-[80%] rounded-lg px-3 py-2 text-sm text-white ${
                        isEven ? 'bg-gray-700' : 'bg-primary'
                      }`}>
                        {m.text}
                      </div>
                    </div>
