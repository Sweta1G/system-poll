...
export default function TeacherPage() {
  ...
  const startPoll = () => {
    ...
    if (!canStart) return;
    socketRef.current.emit("teacher:start", { question: question.trim(), options: opts, duration });
  };

  const endPoll = () => socketRef.current.emit("teacher:end");

  const addOption = () => setOptions((o) => (o.length < 10 ? [...o, ""] : o));
                ...
                {options.map((opt, idx) => (
                  <div key={idx} className="grid grid-cols-2 gap-4 items-center">
                    ...
                    <div className="flex items-center gap-6 text-sm text-muted-foreground">
                      <label className="inline-flex items-center gap-2"><input type="radio" name={`opt-${idx}`} disabled /> Yes</label>
                      <label className="inline-flex items-center gap-2"><input type="radio" name={`opt-${idx}`} defaultChecked disabled /> No</label>
                    </div>
                  </div>
                ))}
              </div>
