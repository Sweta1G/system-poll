    ...
    const handlePollUpdate = (state: any) => {
      ...
      if (window.location.hostname !== 'localhost') {
        console.log('Poll update received:', { 
          ...
          votesLength: state.votes?.length,
          question: state.question?.substring(0, 20) + '...' 
        });
      }
      
      // Store the complete server state for exact teacher matching
      setServerPollState(state);
            ...
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                {currentPoll.options.map((opt: string, i: number) => {
                  // Use exact same calculation as teacher from server state
                  const votes = serverPollState?.votes || [];
                  const total = Math.max(1, votes.reduce((a: number, b: number) => a + b, 0));
