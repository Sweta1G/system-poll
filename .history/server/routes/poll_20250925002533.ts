...
export class PollManager {
  ...
  private onConnection(socket: Socket) {
    socket.on("student:join", (name: string) => {
      console.log('Server received student:join with:', name);
    ...
    socket.on("student:submit", (payload: { optionIndex: number }) => {
      ...
      this.emitState();
      // Remove auto-end logic to allow more students to join and vote
      // Polls will end naturally when timer expires or teacher manually ends
    });

    socket.on("teacher:start", (payload: { question: string; options: string[]; duration?: number }) => {
    ...
    socket.on("disconnect", () => {
      ...
      // Remove auto-end on disconnect to allow continuous participation
      // Polls will end naturally when timer expires or teacher manually ends
      this.emitParticipants();
      this.emitState();
    });
